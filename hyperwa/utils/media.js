...
function cutAudio(buff,start,end){
	let buf;
const media = fs.writeFileSync('./hyperwa/temp/cut.mp3',buff)
	ff(media)
  .setStartTime('00:'+start)
  .setDuration(end)
  .output('./hyperwa/temp/ouputcut.mp3')
  .on('end', function(err) {
    if(!err) {
	buf = fs.readFileSync('./hyperwa/temp/ouputcut.mp3')
	}
  })
...
function cutVideo(buff,start,end){
	let buf;
const media = fs.writeFileSync('./hyperwa/temp/cut.mp4',buff)
	ff(media)
  .setStartTime('00:'+start)
  .setDuration(end)
  .output('./hyperwa/temp/ouputcut.mp4')
  .on('end', function(err) {
    if(!err) {
	buf = fs.readFileSync('./hyperwa/temp/ouputcut.mp4')
	}
  })
  .on('error', err => buf = false)
  return buf
